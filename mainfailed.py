...
@app.route('/')
def home():
    return "Bot Discord đang chạy 24/7 trên Render & MongoDB!"
...
# ==================== 3. HÀM QUẢN LÝ DỮ LIỆU CLOUD (MONGODB) ====================
...
def save_user(user_data: dict):
    """Lưu hoặc cập nhật thông tin người dùng lên MongoDB"""
    uid = str(user_data["_id"])
    collection.update_one({"_id": uid}, {"$set": user_data}, upsert=True)

def delete_user(user_id) -> bool:
    """Xóa thông tin người dùng khỏi MongoDB"""
    res = collection.delete_one({"_id": str(user_id)})
    return res.deleted_count > 0

def get_leaderboard_data():
    """Lấy danh sách tất cả thành viên xếp hạng theo điểm giảm dần"""
    docs = collection.find().sort("points", -1)
    result = []
    for doc in docs:
        uid = doc["_id"]
        info = {
...
async def auto_lock_channel():
    ...
    if vietnam_now.hour == 0 and vietnam_now.minute == 0:
        try:
            channel = await bot.fetch_channel(QUEST_CHANNEL_ID)
            if channel:
            ...
            print(f"Lỗi khóa kênh tự động: {e}")
...
async def unlock_channel(ctx):
    try:
        ...
        await ctx.send(f"❌ **Lỗi Discord:** `{e}`\n*(Hãy kiểm tra lại xem Role của Bot đã nằm trên Role thường chưa!)*")
...
async def unlock_channel_error(ctx, error):
    if isinstance(error, commands.MissingPermissions):
        embed = discord.Embed(
            ...
            description="Bạn cần quyền **Manage Channels (Quản lý kênh)** để dùng lệnh này!", 
            color=discord.Color.red()
        )
        await ctx.send(embed=embed)
    else:
...
async def reset_user(ctx, member: discord.Member):
    success = delete_user(member.id)
    if success:
        embed = discord.Embed(
            title="🔄 RESET DỮ LIỆU THÀNH CÔNG",
            description=f"Toàn bộ dữ liệu KiPoints & stats của {member.mention} đã được xóa khỏi Cloud Database!",
            color=discord.Color.red()
        )
    else:
        embed = discord.Embed(
...
async def refund_user(ctx, member: discord.Member):
    ...
    if user_info.get("total_quests", 0) == 0 or not user_info.get("last_date"):
        embed = discord.Embed(

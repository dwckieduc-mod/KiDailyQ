...
class LeaderboardView(discord.ui.View):
    ...
    def create_embed(self):
        ...
        for index, (user_id, info) in enumerate(page_data, start=start_idx + 1):
            ...
            description += f"{medal} <@{user_id}> - **{points}** KiPoints (Chuỗi: {streak}/3)\n"
    ...
    if has_image:
        ...
        if is_bonus:
            embed.add_field(
                ...
                value="Bạn duy trì chuỗi 3 ngày liên tiếp và nhận thêm **+70 KiPoints Bonus**!", 
                inline=False
            )
        else:
            embed.add_field(
...
async def help_command(ctx):
    ...
    embed.add_field(
        ...
        value=(
            "• `k.add @User <số KiPoints>`: Cộng thêm KiPoints cho thành viên.\n"
            "• `k.remove @User <số KiPoints>`: Trừ KiPoints của thành viên.\n"
            "• `k.reset @User`: Đặt lại toàn bộ KiPoints và streak của thành viên về 0.\n"
    ...
    embed.add_field(
        ...
        value=(
            ...
            "• Duy trì đủ **3 ngày liên tiếp** nhận thưởng thêm **+70 KiPoints bonus** và reset chuỗi mới."
        ),
        inline=False
    )
...
async def refund_user(ctx, member: discord.Member):
    ...
    if is_streak_bonus_day:
        # Vừa nhận thưởng chuỗi 3 ngày -> Trừ 100 KiPoints gốc + 70 KiPoints bonus
        amount = 170
        restored_streak = 2
        bonus_msg = " *(Trừ 100 KiPoints gốc + 70 KiPoints bonus streak)*"
    else:
        # Trường hợp bình thường -> Trừ 100 KiPoints gốc, lùi 1 streak
        amount = 100
    ...
    if isinstance(error, commands.MissingPermissions):
        ...
        embed = discord.Embed(
            ...
            description="Vui lòng nhập đúng cú pháp:\n\n"
                        "• `k.add @User <số_KiPoints>`\n"
                        "• `k.remove @User <số_KiPoints>`\n"
                        "• `k.reset @User`",
            color=discord.Color.gold()
        )
    else:

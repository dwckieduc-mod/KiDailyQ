...
def get_streak_text(streak: int) -> str:
    """Trả về định dạng hiển thị Streak: >=3 hiển thị X ngày, <3 hiển thị Chưa đủ điều kiện"""
    if streak >= 3:
...
async def add_streak(ctx, member: discord.Member, amount: int):
    ...
    embed.add_field(name="🔥 Chuỗi Streak Hiện Tại", value=f"**{get_streak_text(user_info['streak'])}** ({user_info['streak']} ngày)")
    embed.set_footer(text=f"Thực hiện bởi Admin: {ctx.author.display_name}")
    await ctx.send(embed=embed)
...
async def remove_streak(ctx, member: discord.Member, amount: int):
    ...
    embed.add_field(name="🔥 Chuỗi Streak Còn Lại", value=f"**{get_streak_text(user_info['streak'])}** ({user_info['streak']} ngày)")
    embed.set_footer(text=f"Thực hiện bởi Admin: {ctx.author.display_name}")
    await ctx.send(embed=embed)
...
async def refund_user(ctx, member: discord.Member):
    ...
    # Kiểm tra xem lượt điểm danh hôm nay có được cộng bonus hay không (streak >= 3)
    if current_streak >= 3:
        amount = 105
